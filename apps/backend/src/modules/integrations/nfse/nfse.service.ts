...
export class NfseService {
  ...
  private getSoapUrl(): string {
    return this.isProduction()
      ? "https://api.iibr.com.br/3520400/1/soap/producao/rps"
      : "https://api.iibr.com.br/3520400/1/soap/homologacao/rps";
  }
  ...
  private computeIntegridade(rpsXml: string): string {
    // Docs iiBrasil: strip /[^\x20-\x7E]+/g then /[ ]+/g, then append token
    const stripped = rpsXml.replace(/[^\x20-\x7E]+/g, "").replace(/[ ]+/g, "");
    return createHash("sha512").update(stripped + this.getToken(), "utf8").digest("hex");
  }
  ...
  private async consultarViaAuxiliar(numero: string): Promise<{ link?: string | null; codigoVerificacao?: string | null } | null> {
    try {
      const params = new URLSearchParams({
        Metodo: "info_nfse",
        Token: this.getToken(),
        CpfCnpjPrestador: this.getCnpjPrestador(),
        numero,
      });
